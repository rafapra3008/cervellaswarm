"""
Test per scripts.memory.retro.cli - Core.

Coverage focus: save_report e generate_retro orchestration.
Helper tests e main() in test_retro_cli_main.py.
"""
...
def test_generate_retro_saves_report_when_requested(mock_save_report, mock_connect_db):
    ...
    mock_cursor.fetchone.return_value = {
        'total': 0, 'successes': 0, 'failures': 0, 'active': 0
    }
    mock_cursor.fetchall.return_value = []
...
def test_generate_retro_prints_metrics_section(mock_connect_db, capsys):
    ...
    mock_cursor.fetchone.return_value = {
        'total': 50, 'successes': 40, 'failures': 10, 'active': 0
    }
    mock_cursor.fetchall.return_value = []
