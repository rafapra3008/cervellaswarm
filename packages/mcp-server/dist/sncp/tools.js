 ...
 */
import { z } from "zod";
import { readProjectFile, listProjects, searchSncp, KNOWN_PROJECTS, } from "./reader.js";
/**
 * Register all SNCP tools on the MCP server instance.
 */
export function registerSncpTools(server) {
    /**
     * Tool: sncp_read_ripresa
     * Read PROMPT_RIPRESA for a project (session context)
     */
    server.tool("sncp_read_ripresa", "Read the PROMPT_RIPRESA (session context) for a CervellaSwarm project. " +
        ...
        try {
            const { content, path } = await readProjectFile(project, "PROMPT_RIPRESA");
            return {
                content: [
                    {
                        type: "text",
                        text: `# PROMPT_RIPRESA - ${project}\n` +
                            `Source: ${path}\n\n---\n\n` +
                            content,
                    },
                ],
     ...
     */
    server.tool("sncp_list_projects", "List all CervellaSwarm SNCP projects with their available files and status.", {}, {
