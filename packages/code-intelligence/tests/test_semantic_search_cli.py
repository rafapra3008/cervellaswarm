"""
Tests for cervellaswarm_code_intelligence/cli/search_cmd.py
...
def test_main_find_symbol_found(mock_search, mock_stats, capsys):
    ...
    with patch.object(sys, "argv", ["cli.py", "/repo", "login"]):
        with patch("cervellaswarm_code_intelligence.cli.search_cmd.SemanticSearch", return_value=mock_search):
            main()
...
def test_main_valueerror_exception(capsys):
    ...
    with patch.object(sys, "argv", ["cli.py", "/repo", "symbol"]):
        with patch(
            "cervellaswarm_code_intelligence.cli.search_cmd.SemanticSearch",
